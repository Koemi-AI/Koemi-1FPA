...
## Expert dispatch cost, 2026-09-12

Isolated microbenchmark of the expert mixture, not the whole model. The full HERM
step dominates total time at this size, so a change worth 1.9x inside the bank
disappears into the noise of an end-to-end timing. Paired A/B in one process,
legacy and grouped alternating, median of five rounds of eleven timed steps,
4,096 tokens at width 64, forward plus backward, CPU with four threads.

| Experts | Legacy step | Grouped step | Step speedup | Forward speedup |
| ---: | ---: | ---: | ---: | ---: |
| 2 | 41.09 ms | 42.17 ms | 0.97x | 0.93x |
| 4 | 50.55 ms | 47.55 ms | 1.06x | 1.02x |
| 8 | 66.13 ms | 49.90 ms | 1.33x | 1.27x |
| 16 | 109.05 ms | 78.03 ms | 1.40x | 1.07x |
| 32 | 166.53 ms | 100.48 ms | 1.66x | 1.55x |
| 64 | 291.10 ms | 149.31 ms | 1.95x | 1.68x |

Every token reaches exactly one expert in both versions, so the per-token FLOPs
are identical across the whole table. The legacy cost still tripled from two to
thirty-two experts, which is dispatch overhead rather than arithmetic: one
`torch.nonzero`, two `index_select` calls, three `nn.Linear` modules and an
in-place `index_copy_` per expert, over a full clone of the context.

Two experts lose a little because the sort and the occupancy count are not
amortized. That is the intended trade: a two-expert bank is not why the code
exists.

An intermediate version indexed the stacked weights directly as
`gate_weight[expert_index]`. It was slower than the loop it replaced, 0.17x at two
experts, because each slice is a `select` node whose backward allocates a gradient
buffer the size of the entire bank. Reading the weights through `unbind` once per
forward fixed it. The lesson is in the numbers: the forward improved in both
versions, and only a forward-plus-backward measurement showed which one was real.

## Expert routing probe, 2026-09-12

Three seeds (17, 29, 41), eight experts, 96 training records, 48 validation
records, three epochs, sequence 64, batch 8, `embedding_size=32`, CPU fp32. The
corpus is six fixed question-answer templates, so it is a memorization task, not a
language-modelling one. Every configuration saw the same corpus and the same
budget.

| Configuration | Parameters | Validation bpb | Seed std | Balance term | Busiest expert | Train tok/s |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| No expert bank | 27,724 | 2.4622 | 0.0937 | - | - | 1,726.3 |
| Hash, top-1 | 78,188 | 1.4478 | 0.1923 | - | 15.2% | 1,351.7 |
| Learned, top-1, no balance term | 78,444 | 2.1528 | 0.0939 | 1.4121 | 50.6% | 1,436.4 |
| Learned, top-1, balance 0.01 | 78,444 | 2.0990 | 0.1265 | 1.0653 | 13.9% | 1,403.0 |
| Learned, top-2, balance 0.01 | 78,444 | 1.9788 | 0.1071 | 1.1105 | 31.9% | 1,379.0 |
| Learned, top-2, balance 0.01, jitter 0.1 | 78,444 | 1.9533 | 0.1255 | 1.1054 | 31.9% | 1,292.4 |

Uniform occupancy would put 12.5% on the busiest of eight experts.

What the numbers support:

The balance term does what it claims. Turning it on moved the auxiliary value from
1.4121 to 1.0653 and the busiest expert from 50.6% of tokens to 13.9%, against a
12.5% uniform floor. No expert was ever idle in any configuration. That is the
mechanism verified, not a quality claim.

Hash dispatch reached lower bits per byte than every learned configuration. The
best learned result, 1.9533, sits 0.505 bpb above the hash result of 1.4478, and
twice the combined seed deviation is 0.459, so the gap clears the noise threshold
by a small margin. At this budget the learned gate costs quality.

That is a statement about this budget, not about learned routing. Three epochs
over six templates is the regime where a router has to learn dispatch and experts
at once with roughly ten thousand supervised tokens, while a hash of byte,
previous byte and position hands out a near-uniform partition for free. It does
not predict what happens at a real corpus and a saturated budget.

Top-2 against top-1 under the balance term, 1.9788 against 2.0990, and jitter
against no jitter, 1.9533 against 1.9788, are both inside twice the combined
deviation. Neither is a demonstrated gain.

Learned routing did not cost throughput: 1,403.0 tokens per second against
1,351.7 for hash is inside the run-to-run spread on a loaded four-thread host. The
gate is one matmul against the vocabulary-sized head and the memory scan.

What the numbers do not support:

The bank against no bank is not a parameter-matched comparison. 78,188 against
27,724 parameters is 2.8x, so the 1.01 bpb gap is at least partly capacity rather
than mixture structure. A parameter-matched control needs a wider dense model and
has not been run.
